    ...
    class Program
    {
        static void Main()
        {
            ...
            while (true)
            {
                ...
                if (choice == 1)
                {
                    CollectAndAddPayment(service, options, emailService);
                }
        ...
        static void CollectAndAddPayment(PaymentShippingService.PaymentShippingService service, string[] options, EmailService emailService)
        {
            Console.WriteLine("\nSelect Method:");
            ...
            else
            {
                ...
                accountIdentifier = input;
            }

            Console.Write("Enter email to send confirmation to: ");
            string recipientEmail = Console.ReadLine();

            try
            {
                emailService.SendEmail(accountIdentifier, recipientEmail);
                Console.WriteLine("✅ Confirmation email sent!");
            }
            catch (Exception ex)
            {
                Console.WriteLine("⚠️ Email failed to send: " + ex.Message);
            }
